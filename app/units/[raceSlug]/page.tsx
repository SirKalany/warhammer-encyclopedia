...
import { UnitSummaryDTO, UnitRole } from "@/lib/types";
...
export default function UnitRosterPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setRaceId(race.id);
      const data = await api.units.findByRace(race.id);
      setUnits(data);
      setLoading(false);
    }
    load();
  ...
  const grouped = ROLES.reduce(
    ...
    {} as Record<UnitRole, UnitSummaryDTO[]>,
  );
          ...
          {Object.entries(grouped).map(([role, roleUnits]) => (
            <div key={role}>
              <h2 className="text-sm font-display tracking-widest uppercase text-text-muted mb-3 border-b border-border-subtle pb-2">
                ...
                <span className="ml-2 text-text-muted font-body normal-case tracking-normal">
                  ({roleUnits.length})
                </span>
              </h2>
